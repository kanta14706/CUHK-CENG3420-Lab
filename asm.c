...
int inst_to_binary(
    ...
    if (is_opcode(opcode) == ADDI) {
         ...
         */
        binary = (0x05 << 12) + (0x04 << 2) + 0x03; // funct3 + opcode
         ...
         * tip: you may need the function `lower5bit`
         */
        binary = (0x10 << 26) + (0x05 << 12) + (0x04 << 2) + 0x03; // imm + funct3 + opcode
		binary += (reg_to_num(arg1, line_no) << 7); // destination reg
		binary += (reg_to_num(arg2, line_no) << 15); // source reg (rs1)
		binary += (lower5bit(arg3, line_no) << 20);
        ...
        /* Lab2-1 assignment */
        binary = (0x07 << 12) + (0x04 << 2) + 0x03; // funct3 + opcode
        binary += (reg_to_num(arg1, line_no) << 7); // destination reg
        binary += (reg_to_num(arg2, line_no) << 15); // source reg (rs1)
        binary += (MASK11_0(validate_imm(arg3, 12, line_no)) << 20);
    ...
    // Conditional Branches
        ...
        /* Lab2-1 assignment */
        binary = (0x01 << 12) + (0x18 << 2) + 0x03; // funct3 + opcode
		binary += (reg_to_num(arg1, line_no) << 15); // rs1
		binary += (reg_to_num(arg2, line_no) << 20); // rs2
        ...
        /* Lab2-1 assignment */
        binary = (0x04 << 12) + (0x18 << 2) + 0x03; // funct3 + opcode
        binary += (reg_to_num(arg1, line_no) << 15); // rs1
        binary += (reg_to_num(arg2, line_no) << 20); // rs2
		int val = label_to_num(line_no, arg3, 12, label_table, number_of_labels);
        int offset = val - addr;
        ...
        /* Lab2-1 assignment */
        binary = (0x05 << 12) + (0x18 << 2) + 0x03; // funct3 + opcode
        binary += (reg_to_num(arg1, line_no) << 15); // rs1
        binary += (reg_to_num(arg2, line_no) << 20); // rs2
		int val = label_to_num(line_no, arg3, 12, label_table, number_of_labels);
        int offset = val - addr;
    ...
    // Load and Store Instructions
        ...
        /* Lab2-1 assignment */
        binary = (0x01 << 12) + 0x03; // funct3 + opcode
        binary += (reg_to_num(arg1, line_no) << 7);
        struct_regs_indirect_addr* ret = parse_regs_indirect_addr(arg2, line_no);
        binary += (reg_to_num(ret->reg, line_no) << 15);
        binary += (MASK11_0(ret->imm) << 20);
        ...
        /* Lab2-1 assignment */
        binary = (0x02 << 12) + 0x03; // funct3 + opcode
		binary += (reg_to_num(arg1, line_no) << 7);
        struct_regs_indirect_addr* ret = parse_regs_indirect_addr(arg2, line_no);
        binary += (reg_to_num(ret->reg, line_no) << 15);
        binary += (MASK11_0(ret->imm) << 20);
